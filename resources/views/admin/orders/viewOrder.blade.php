    ...
    <section class="pt-3 pb-5">
        <div class="container pb-5">
            <div class="row my-2 product-Data">
                <div class="col-sm-6">
                    <h2 class="fw-bold text-danger">Shipping Details</h2>
                    <hr>
                    <h4>
                        ...
                        <span class="fw-bold text-danger">Status:</span>&nbsp;<br>
                        <form method="post" action="{{url('UpdateStatus/'.$order[0]->checkout->id)}}">
                        @csrf
                        {{method_field("PUT")}}
                        <div class="row">
                            <div class="col-6">
                                <select class="form-select"  name="OrderStatus">
                                    <option selected>Opne This Select Menu</option>
                                    <option {{$order[0]->checkout->status==0?'selected':''}} value="0">Pending</option>
                                    <option {{$order[0]->checkout->status==1?'selected':''}} value="1">Completed</option>
                                </select>
                            </div>
                            <div class="col-6">
                                <button type="submit" class="btn btn-danger"><i class="fa-solid fa-pen-nib"></i></button>
                            </div>
                        </div>
                        </form>
                ...
                <div class="col-sm-6">
                    <h2 class="fw-bold text-danger">Order Details</h2>
                    <hr>
                    ...
                    @endforeach
                    <a href="{{ url('OrderDetails') }}" class="btn btn-warning text-white float-end my-2 btn-lg">Go Back</a>
                </div>
            </div>
